...
def test_slack_to_markdown_link_conversion() -> None:
    text = "Check <https://grafana.net|Grafana> for logs or view <https://google.com>. \x1b[2mLog snippet\x1b[0m"
    converted = _slack_to_markdown(text)
    assert (
        converted
        == "Check [Grafana](https://grafana.net) for logs or view [https://google.com](https://google.com). Log snippet"
    )
...
def test_generate_report_calls_create_trello_card_when_configured() -> None:
    ...
    with (
        patch("app.utils.slack_delivery.send_slack_report", return_value=(True, None)),
...
def test_generate_report_resolves_list_via_board_id_and_pipeline_name() -> None:
    ...
    with (
        ...
        patch("app.delivery.publish_findings.node.open_in_editor"),
        patch("app.delivery.publish_findings.node.render_report"),
        patch("app.integrations.trello.get_trello_board") as mock_get_board,
        ...
        mock_get_board.return_value = {"id": "long-board-id"}
        ...
        # Should verify board is fetched to get long ID
        mock_get_board.assert_called_once_with(config=ANY, board_id="test-board")
        # Should verify get_trello_board_lists is called with the resolved long board ID
        mock_get_lists.assert_called_once_with(config=ANY, board_id="long-board-id")
        # Since 'vauthz-sync-issue' is not in existing lists, it should create the list
        mock_create_list.assert_called_once_with(
            config=ANY, board_id="long-board-id", name="vauthz-sync-issue"
        )
