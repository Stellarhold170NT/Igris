...
def test_slack_to_markdown_link_conversion() -> None:
    ...
    assert converted == "Check [Grafana](https://grafana.net) for logs or view [https://google.com](https://google.com). Log snippet"
...
def test_generate_report_resolves_list_via_board_id_and_pipeline_name() -> None:
    ...
    with patch("app.utils.slack_delivery.send_slack_report", return_value=(True, None)), \
        ...
        mock_create_list.assert_called_once_with(config=ANY, board_id="long-board-id", name="vauthz-sync-issue")
